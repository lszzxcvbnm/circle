...
import com.circlett.demo.utils.JwtUtils;
...
public class JwtAuthenticationFilter extends BasicAuthenticationFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (StrUtil.isBlankOrUndefined(jwt)) {//判断是否为空
			chain.doFilter(request, response);//为空过滤器链向后走
			return;
		}
		...
		UsernamePasswordAuthenticationToken token
				= new UsernamePasswordAuthenticationToken(username, null, userDetailService.getUserAuthority(sysUser.getIsAdmin()));
